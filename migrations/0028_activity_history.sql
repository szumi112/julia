...
CREATE TABLE activity_history_details (
  audit_id TEXT PRIMARY KEY NOT NULL
    REFERENCES audit_events(id) ON UPDATE RESTRICT ON DELETE RESTRICT,
  scope_type TEXT NOT NULL,
  scope_id TEXT NOT NULL,
  scope_purpose TEXT NOT NULL,
  details_envelope TEXT NOT NULL CHECK (json_valid(details_envelope)),
  CHECK (
    (scope_type='client' AND scope_id LIKE 'cl_%' AND scope_purpose='identity')
    OR (scope_type='centre_finance' AND scope_id='centre_1' AND scope_purpose='ledger')
  )
);

CREATE TRIGGER activity_history_details_no_update
BEFORE UPDATE ON activity_history_details
BEGIN
  SELECT RAISE(ABORT, 'append_only');
END;

CREATE TRIGGER activity_history_details_no_delete
BEFORE DELETE ON activity_history_details
BEGIN
  SELECT RAISE(ABORT, 'append_only');
END;

-- Expand the capability vocabulary in both durable authority tables. SQLite
-- cannot alter a CHECK constraint, so copy the rows into matching new tables.
CREATE TABLE staff_capability_overrides_new (
  ...
  updated_at TEXT NOT NULL CHECK (length(updated_at)>0 AND updated_at>=created_at),
  PRIMARY KEY (staff_id,capability)
);

INSERT INTO staff_capability_overrides_new
...
DROP TABLE staff_capability_overrides;
ALTER TABLE staff_capability_overrides_new RENAME TO staff_capability_overrides;
...
CREATE TABLE staff_capability_override_history_new (
  ...
  role_at_change TEXT NOT NULL CHECK (role_at_change IN ('owner','coordinator','specialist')),
  decision TEXT NOT NULL CHECK (decision IN ('allow','deny','cleared')),
  ...
  changed_by_staff_id TEXT NOT NULL REFERENCES staff_users(id)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  reason TEXT NOT NULL CHECK (reason IN ('owner_update','role_change','status_change')),
  changed_at TEXT NOT NULL CHECK (length(changed_at)>0)
);

INSERT INTO staff_capability_override_history_new
  (id,staff_id,capability,role_at_change,decision,override_version,
   authority_revision,changed_by_staff_id,reason,changed_at)
SELECT id,staff_id,capability,role_at_change,decision,override_version,
       authority_revision,changed_by_staff_id,reason,changed_at
FROM staff_capability_override_history;

DROP TABLE staff_capability_override_history;
ALTER TABLE staff_capability_override_history_new
  RENAME TO staff_capability_override_history;
