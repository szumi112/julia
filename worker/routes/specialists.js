...
export async function postSpecialistProfile(input) {
  try {
    ...
    const match = error instanceof TypeError
      ? /^VALIDATION_FAILED\/(body|displayName|professionalTitle|standardRateGrosze|longRateGrosze|specialization|avatarKey)$/.exec(error.message)
      : null
...
export async function postSpecialistProfileEdit(input) {
  try {
    ...
    const match = error instanceof TypeError
      ? /^VALIDATION_FAILED\/(body|displayName|professionalTitle|standardRateGrosze|longRateGrosze|specialization|avatarKey|expectedVersion)$/.exec(error.message)
      : null
    if (match) throw new AppError('VALIDATION_FAILED', { field: match[1] })
    throw error
  }
