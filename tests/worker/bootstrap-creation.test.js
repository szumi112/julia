...
it('normalizes every exact Phase 1 identity audit shape without mutating the row', () => {
  for (const [action, fixture] of Object.entries(LEGACY_IDENTITY_AUDITS)) {
...
it('accepts exact new identity and null-actor system audit shapes only', () => {
  const identity = auditRow(
    'identity.activation',
    'staff_user',
    { invitationVersion: 2, specialistVersion: 3, staffVersion: 2 },
  )
  expect(normalizeBootstrapAuditEvent(identity).metadata.specialistVersion).toBe(3)
