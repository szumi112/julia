...
describe('closed core route descriptors', () => {
  ...
  it('publishes only immutable nonfunctional route metadata with registered audit actions', () => {
    expect(CORE_ROUTE_DESCRIPTORS.map(({ id, capability, auditActions, bodyKeys, sharedBudget }) => ({
      ...
      { id: 'specialists.create', capability: 'staff.manage', auditActions: ['specialist.profile.created'], bodyKeys: ['displayName', 'professionalTitle', 'standardRateGrosze', 'longRateGrosze', 'specialization', 'avatarKey'], sharedBudget: { totalLimit: 50, recoveryReserve: 8 } },
      { id: 'specialists.edit', capability: 'staff.manage', auditActions: ['specialist.profile.updated'], bodyKeys: ['expectedVersion', 'displayName', 'professionalTitle', 'standardRateGrosze', 'longRateGrosze', 'specialization', 'avatarKey'], sharedBudget: { totalLimit: 50, recoveryReserve: 8 } },
      ...
      { id: 'finance.entry.detail', capability: 'finance.centre.manage', auditActions: [], bodyKeys: null, sharedBudget: { totalLimit: 50, recoveryReserve: 8 } },
      { id: 'finance.entry.create', capability: 'finance.centre.manage', auditActions: ['finance.entry.created'], bodyKeys: ['kind', 'recordType', 'accountingMonth', 'occurredOn', 'amountGrosze', 'paidAmountGrosze', 'paymentMethod', 'settlementStatus', 'invoiceStatus', 'counterparty', 'sourceLabel', 'invoiceNote', 'specialistId', 'lessonCount', 'source'], sharedBudget: { totalLimit: 50, recoveryReserve: 8 } },
      { id: 'finance.entry.adjust', capability: 'finance.centre.manage', auditActions: ['finance.entry.adjusted'], bodyKeys: ['expectedVersion', 'reason', 'accountingMonth', 'paidAmountGrosze', 'paymentMethod', 'settlementStatus', 'invoiceStatus'], sharedBudget: { totalLimit: 50, recoveryReserve: 8 } },
