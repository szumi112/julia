...
test('@owner keeps invalid manual finance input editable without sending or locking a save', async ({ page }) => {
  ...
  await expect(drawer.getByLabel('Za co?')).toHaveAttribute('placeholder', 'np. warsztaty dla rodziców')
  await drawer.getByRole('button', { name: 'Dodaj przychód', exact: true }).click()
  await expect(drawer.getByLabel('Za co?')).toHaveAttribute('aria-invalid', 'true')
  ...
  await expect(drawer.getByLabel('Za co?')).toBeFocused()
  expect(requests).toEqual([])
  await drawer.getByLabel('Za co?').fill('Opłata za zajęcia')
  ...
  await expect(drawer.getByLabel('Za co?')).toBeEnabled()
  await expect(drawer.getByRole('button', { name: 'Anuluj', exact: true })).toBeEnabled()
  await expect(drawer.getByRole('button', { name: 'Spróbuj ponownie' })).toHaveCount(0)
  expect(requests).toEqual([])
  await drawer.getByLabel('Za co?').fill('Opłata za zajęcia')
  await expect(drawer.getByLabel('Za co?')).toHaveValue('Opłata za zajęcia')
})
...
test('@owner lets a new entry follow its date until the accounting month is selected manually', async ({ page }) => {
  ...
  await drawer.getByLabel('Data (opcjonalnie)').fill('2026-08-14')
  await expect(month).toHaveValue('2026-08')
  await month.selectOption('2026-07')
  await drawer.getByLabel('Data (opcjonalnie)').fill('2026-09-02')
...
test('@owner UI contract creates a compact expense and retains its settlement classification on correction', async ({ page }) => {
  ...
  await drawer.getByLabel('Za co?').fill('Fikcyjny zakup pomocy edukacyjnych')
  await drawer.getByLabel('Kwota (zł)', { exact: true }).fill('123,45')
  await drawer.getByRole('button', { name: 'Dodaj wydatek', exact: true }).click()
  await expect(drawer.getByRole('alert')).toContainText('Nie mamy pewności')
...
test('@owner UI contract bills individual English lessons and rejects a duplicate monthly charge', async ({ page }) => {
  ...
  await participant.getByRole('button', { name: 'Dodaj uczestnika' }).click()
...
test('@owner UI contract bills a TUS participant through an explicit group membership', async ({ page }) => {
  ...
  const month = new URLSearchParams(new URL(page.url()).hash.split('?')[1]).get('ym')
