...
test('@owner reconciles POST payment and correction mutations in the fictional reload workflow', async ({ page }) => {
  ...
  await correction.getByLabel('Data wpłaty').fill('2026-08-05')
...
test('@specialist keeps her own selection when the client’s latest session belongs to another specialist', async ({ page }) => {
  ...
  await page.locator('.id-band__actions').getByRole('button', { name: 'Przygotuj sesję' }).click()
...
test('@owner keeps archive conflict explicit until cancellation makes archive legal', async ({ page }) => {
  ...
  await expect(drawer.getByRole('alert')).toContainText('Ten klient ma zaplanowane sesje. Odwołaj je, zanim zarchiwizujesz klienta.')
...
test('@owner discards a stale second drawer while an offline draft remains open', async ({ page, context }) => {
  ...
  const secondDrawer = second.getByRole('dialog', { name: 'Edycja klienta' })
  await secondDrawer.getByRole('button', { name: 'Zapisz zmiany' }).click()
  await expect(secondDrawer.getByRole('alert')).toContainText('Ktoś w międzyczasie zmienił dane klienta')
  await expect(secondDrawer.getByLabel('Imię i nazwisko')).toHaveValue('Ola Przestarzała')
  await secondDrawer.getByRole('button', { name: 'Wczytaj aktualne dane' }).click()
  await expect(secondDrawer.getByLabel('Imię i nazwisko')).toHaveValue('Ola Zwycięska')
  expect(edits).toBe(2)
